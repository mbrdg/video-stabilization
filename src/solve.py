...
def compute_crop_window(
    frame_shape: Tuple[float, float], crop_ratio: float
) -> np.ndarray:
    """
...
def stabilize(
    ...
    e1 = pulp.LpVariable.dicts(
        "e1",
        ((i, j) for i in range(number_of_frames) for j in range(dof)),
        lowBound=0.0,
    )
    ...
    problem += (
        w1
        * pulp.lpSum(
            [e1[i, j] * c[j] for i in range(number_of_frames) for j in range(dof)]
        )
        + w2
        * pulp.lpSum(
            [e2[i, j] * c[j] for i in range(number_of_frames) for j in range(dof)]
        )
        + w3
        * pulp.lpSum(
            [e3[i, j] * c[j] for i in range(number_of_frames) for j in range(dof)]
        )
    )
    ...
    for ts in range(number_of_frames - 3):
        residual = [
            (
                p[ts + 1, 0]
                + frame_transforms[ts + 1, 2, 0] * p[ts + 1, 2]
                + frame_transforms[ts + 1, 2, 1] * p[ts + 1, 3]
            )
            - p[ts, 0],
            (
                p[ts + 1, 0]
                + frame_transforms[ts + 1, 2, 0] * p[ts + 1, 4]
                + frame_transforms[ts + 1, 2, 1] * p[ts + 1, 5]
            )
            - p[ts, 1],
            (
                frame_transforms[ts + 1, 0, 0] * p[ts + 1, 2]
                + frame_transforms[ts + 1, 0, 0] * p[ts + 1, 3]
            )
            - p[ts, 2],
            (
                frame_transforms[ts + 1, 1, 0] * p[ts + 1, 2]
                + frame_transforms[ts + 1, 1, 0] * p[ts + 1, 3]
            )
            - p[ts, 3],
            (
                frame_transforms[ts + 1, 0, 0] * p[ts + 1, 2]
                + frame_transforms[ts + 1, 0, 0] * p[ts + 1, 5]
            )
            - p[ts, 4],
            (
                frame_transforms[ts + 1, 1, 0] * p[ts + 1, 2]
                + frame_transforms[ts + 1, 1, 0] * p[ts + 1, 5]
            )
            - p[ts, 5],
        ]
        ...
        residual_d1 = [
            (
                p[ts + 2, 0]
                + frame_transforms[ts + 2, 2, 0] * p[ts + 2, 2]
                + frame_transforms[ts + 2, 2, 1] * p[ts + 2, 3]
            )
            - p[ts + 1, 0],
            (
                p[ts + 2, 0]
                + frame_transforms[ts + 2, 2, 0] * p[ts + 2, 4]
                + frame_transforms[ts + 2, 2, 1] * p[ts + 2, 5]
            )
            - p[ts + 1, 1],
            (
                frame_transforms[ts + 2, 0, 0] * p[ts + 2, 2]
                + frame_transforms[ts + 2, 0, 0] * p[ts + 2, 3]
            )
            - p[ts + 1, 2],
            (
                frame_transforms[ts + 2, 1, 0] * p[ts + 2, 2]
                + frame_transforms[ts + 2, 1, 0] * p[ts + 2, 3]
            )
            - p[ts + 1, 3],
            (
                frame_transforms[ts + 2, 0, 0] * p[ts + 2, 2]
                + frame_transforms[ts + 2, 0, 0] * p[ts + 2, 5]
            )
            - p[ts + 1, 4],
            (
                frame_transforms[ts + 2, 1, 0] * p[ts + 2, 2]
                + frame_transforms[ts + 2, 1, 0] * p[ts + 2, 5]
            )
            - p[ts + 1, 5],
        ]
        ...
        residual_d2 = [
            (
                p[ts + 3, 0]
                + frame_transforms[ts + 3, 2, 0] * p[ts + 3, 2]
                + frame_transforms[ts + 3, 2, 1] * p[ts + 3, 3]
            )
            - p[ts + 2, 0],
            (
                p[ts + 3, 0]
                + frame_transforms[ts + 3, 2, 0] * p[ts + 3, 4]
                + frame_transforms[ts + 3, 2, 1] * p[ts + 3, 5]
            )
            - p[ts + 2, 1],
            (
                frame_transforms[ts + 3, 0, 0] * p[ts + 3, 2]
                + frame_transforms[ts + 3, 0, 0] * p[ts + 3, 3]
            )
            - p[ts + 2, 2],
            (
                frame_transforms[ts + 3, 1, 0] * p[ts + 3, 2]
                + frame_transforms[ts + 3, 1, 0] * p[ts + 3, 3]
            )
            - p[ts + 2, 3],
            (
                frame_transforms[ts + 3, 0, 0] * p[ts + 3, 2]
                + frame_transforms[ts + 3, 0, 0] * p[ts + 3, 5]
            )
            - p[ts + 2, 4],
            (
                frame_transforms[ts + 3, 1, 0] * p[ts + 3, 2]
                + frame_transforms[ts + 3, 1, 0] * p[ts + 3, 5]
            )
            - p[ts + 2, 5],
        ]
        ...
        for d in range(dof):
            ...
            problem += (
                -e3[ts, d]
                <= residual_d2[d] - 2 * residual_d1[d] + residual[d]
                <= e3[ts, d]
            )
