...
def optical_flow(video: str) -> None:
    ...
    while capture.isOpened():
        ...
        curr_pts, status, _ = cv2.calcOpticalFlowPyrLK(prev_gray_frame, gray_frame, prev_pts, None, **lk_params)
        ...
        for i, (new, old) in enumerate(zip(next_pts, curr_pts)):
            a, b = new.ravel()
            c, d = old.ravel()
            drawing_mask = cv2.line(drawing_mask, (int(a), int(b)), (int(c), int(d)), tracking_colors[i].tolist(), 2)
            frame = cv2.circle(frame, (int(a), int(b)), 5, tracking_colors[i].tolist(), -1)
